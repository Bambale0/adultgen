...
def test_production_compose_declares_web_and_api_service_graph() -> None:
    ...
    assert "apps/web_app/Dockerfile" in compose
    assert "VITE_GOOGLE_CLIENT_ID" in compose
    assert "VITE_TELEGRAM_BOT_USERNAME" in compose
...
def test_nginx_config_routes_api_and_spa() -> None:
    gateway = read("deploy/nginx/gateway.conf")

    assert "location /api/" in gateway
    assert "proxy_pass http://backend:8000/" in gateway
    assert "proxy_pass http://adultgen_web" in gateway
    assert "server web:8080" in gateway
...
def test_ci_runs_web_tests_and_build_before_backend_merge() -> None:
    ...
    assert ci.index("npm run test") < ci.index("npm run build")


def test_deployment_runbook_documents_web_and_api_operational_flows() -> None:
    runbook = read("docs/PRODUCTION_DEPLOYMENT.md")

    assert "new React/Vite frontend" in runbook
    assert "Google Identity Services" in runbook
    ...
    assert "TELEGRAM_LOGIN_BOT_USERNAME" in runbook
    assert "Do not restore the rejected frontend" in runbook
