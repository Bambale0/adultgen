...
async def test_local_object_storage_can_read_back_uploaded_bytes(tmp_path: Path) -> None:
    ...
    await storage.put_object(
        bucket="media-published",
        key="published/example.txt",
        body=b"hello",
...
def test_orbital_web_knows_core_media_urls() -> None:
    ...
    assert "mediaUrl(path: string)" in content
    assert "path.startsWith('http') ? path : `${API}${path}`" in content
